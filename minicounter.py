...
if __name__ == '__main__':
    count = Countercomm('/dev/serial/by-id/usb-Centre_for_Quantum_Technologies_USB_Counter_Ucnt-QO10-if00')
    count.set_NIM()
    gate = 5
    count.set_gate_time(gate)
    i = 0
    import time
    timenow = time.time()
    counter = 0
    while True:
        print count.get_all_counts()
        now = time.time()
        timenow = now
        i +=1
        if i> 1000:
